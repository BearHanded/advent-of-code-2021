...
def oxygen_filter(o_row, idx, curr_sums, curr_gamma):
    most_common_bit = curr_gamma[idx] if curr_sums[idx][0] != curr_sums[idx][1] else 1
    return o_row[idx] == str(most_common_bit)


def carbon_filter(c_row, idx, curr_sums, curr_epsilon):
    least_common_bit = curr_epsilon[idx] if curr_sums[idx][0] != curr_sums[idx][1] else 0
